...
class EventEmotionTrigger:
    ...
    def _initialize_triggers(self) -> List[EventTrigger]:
        ...
        triggers = [
            ...
            # Criticism dan feedback negatif
            EventTrigger(
            ...
            EventTrigger(
                ...
                emotion_type=EmotionType.CURIOUS,
                intensity_base=0.7,
                duration_base=100.0,
                intensity_modifiers={"novelty": 1.3, "relevance": 1.1}
            ),
            
            # Confusion dan clarity
            EventTrigger(
            ...
            EventTrigger(
                ...
                emotion_type=EmotionType.EMPATHETIC,
                intensity_base=0.7,
                duration_base=120.0,
    ...
    def _calculate_trigger_duration(self, trigger: EventTrigger, context: Dict[str, Any] = None) -> float:
        ...
        if (trigger.emotion_type in [EmotionType.HAPPY, EmotionType.EXCITED] and 
            context.get("user_satisfaction", 0) > 0.8):
            duration_modifier *= 1.3
            
        # Durasi lebih pendek untuk emosi negatif jika ada resolusi cepat
        if (trigger.emotion_type in [EmotionType.SAD, EmotionType.ANGRY] and 
    ...
    def _apply_adaptive_intensity(self, intensity: float, event_type: EventType, 
        ...
        for emotion_data in current_emotions.get("emotions", []):
    ...
    def get_trigger_statistics(self) -> Dict[str, Any]:
        ...
        for event_record in self.event_history:
            ...
            # Hitung yang recent (1 jam terakhir)
        ...
        for event_type in event_counts:
            intensities = [
                r["intensity"] for r in self.event_history 
                if r["event_type"] == event_type
            ]
            if intensities:
                event_counts[event_type]["avg_intensity"] = sum(intensities) / len(intensities)
        ...
        return {
            ...
            "session_duration": time.time() - (self.event_history[0]["timestamp"] if self.event_history else time.time())
        }
    
    def set_sensitivity(self, sensitivity: float) -> None:
        """Mengatur sensitivitas trigger (0.0 - 1.0)"""
        self.config["trigger_sensitivity"] = max(0.0, min(1.0, sensitivity))
    
    def enable_pattern_matching(self, enabled: bool) -> None:
        """Mengaktifkan/menonaktifkan pattern matching"""
        self.config["pattern_matching_enabled"] = enabled
    ...
    def export_event_data(self) -> Dict[str, Any]:
        """Mengekspor data event untuk analisis"""
        return {
            "event_history": self.event_history.copy(),
            "trigger_config": self.config.copy(),
            "interaction_stats": {
...
if __name__ == "__main__":
    ...
    event_trigger = EventEmotionTrigger(emotion_manager)
    ...
    user_inputs = [
        ...
        "This is wrong and doesn't make sense",
        "Wow, that's amazing! I'm excited to try it!"
    ]
    ...
    for user_input in user_inputs:
        ...
        # Proses interaksi
        processed_events = event_trigger.process_user_interaction(
            user_input=user_input,
            context={"user_satisfaction": 0.8}
        )
        
        print(f"Detected events: {[e.value for e in processed_events]}")
        
        # Lihat state emosi saat ini
        emotional_state = emotion_manager.get_current_emotional_state()
        print(f"Current emotions: {[e['type'] for e in emotional_state['emotions']]}")
        print(f"Current mood: {emotional_state['mood']['type']}")
    
    # Statistik
    stats = event_trigger.get_trigger_statistics()
    print(f"\nSession statistics:")
    print(f"Total events: {stats['total_events']}")
    print(f"Most common events: {list(stats['event_breakdown'].keys())[:3]}")